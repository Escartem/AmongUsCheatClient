	...
	void RenderRadar(bool* state, float radarZoom) {
		...
		if (IsInGame())
		{
			...
			drawList->AddCircleFilled(ImVec2(winpos.x + winsize.x * 0.5F, winpos.y + winsize.y * 0.5F),
				CCConstants::ICON_SCALE, GetRadarPlayerColor(localPlayer));
			drawList->AddCircle(ImVec2(winpos.x + winsize.x * 0.5F, winpos.y + winsize.y * 0.5F),
				CCConstants::ICON_SCALE + 0.5f, GetRadarPlayerColorStatus(localPlayer), 0, 2.0F);
			...
			for (auto player : GetAllPlayers()) {
				...
				drawList->AddCircleFilled(ImVec2(radX, radY), CCConstants::ICON_SCALE, GetRadarPlayerColor(player));
				drawList->AddCircle(ImVec2(radX, radY), CCConstants::ICON_SCALE + 0.5F, GetRadarPlayerColorStatus(player), 0, 2.0F);
			}
		}
		ImGui::End();
	}
